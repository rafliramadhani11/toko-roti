<nav x-data="{ open: false }" class="relative w-full bg-white rounded shadow">
    <div class="flex items-center justify-between px-3 py-3 md:px-8 lg:max-w-7xl lg:mx-auto">
        {{-- APP NAME --}}
        <div>
            <a href="{{ route('guest.index') }}" wire:navigate class="text-2xl font-bold">Delicia Bakery</a>
        </div>

        {{-- LINK --}}
        <div x-data="{ dropdown: false }" class="relative hidden md:flex gap-x-3">
            <div class="pr-3 border-r-2 border-slate-300">
                <x-link href="{{ route('admin.app') }}" wire:navigate
    ...
    <x-mobile-dropdown x-show="open" x-cloak>
        ...
        <x-mobile-link href="{{ route('admin.app') }}"
            class="{{ Route::is('admin.app') ? 'bg-slate-200' : '' }} flex items-center">
            <x-icons.box />
            Stok Barang
        </x-mobile-link>
        ...
        <form action="{{ route('admin.logout') }}" method="post">
            @csrf
            <button
                class="flex items-center w-full px-3 py-2 mb-1 font-semibold transition duration-300 bg-white rounded-none hover:text-white text-slate-900 hover:bg-red-500">
                <x-icons.sign-out />
                Sign Out
            </button>
        </form>
